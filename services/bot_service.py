...
class DiscordBotService:
    ...
    async def get_all_guilds_info(self) -> bool:
        ...
        if not self.bot.is_ready():
            self.event_manager.publish("ERROR", "[오류] 봇이 Discord에 연결될 때까지 기다려 주세요.") # Error Event pub
            return False
        # Discord Bot 객체의 guilds 속성에서 직접 정보를 가져옵니다.
        self.app_state.all_guilds = list(self.bot.guilds)
        self.event_manager.publish("GUILDS_UPDATED") # Guilds updated Event pub
        return True
    ...
    async def select_guild(self, value: str) -> bool:
        ...
        if guild_found:
            ...
            self.event_manager.publish("GUILD_SELECTED", guild_found.name) # Guild selected Event pub
            return True
        return False
    ...
    async def select_channel(self, value: str) -> bool:
        ...
        if not self.app_state.current_guild:
            self.event_manager.publish("ERROR", "[오류] 채널을 설정하려면 먼저 서버를 선택해 주세요.") # Error Event pub
            return False
        ...
        if channel_found and isinstance(channel_found, discord.TextChannel):
            ...
            self.event_manager.publish("CHANNEL_SELECTED", channel_found.name) # Channel selected Event pub
            return True
        return False
    ...
    async def fetch_recent_messages(self, count: int = 20) -> bool:
        ...
        if not self.app_state.current_channel:
            self.event_manager.publish("ERROR", "[오류] 먼저 채널을 선택해 주세요.") # Error Event pub
            return False
        ...
        try:
            ...
            self.event_manager.publish("ERROR", "[오류] 채널 메시지 읽기 권한이 없습니다. 봇 역할 권한을 확인해 주세요.") # Error Event pub
        except Exception as e:
            self.event_manager.publish("ERROR", f"[오류] 메시지 가져오기 실패: {e}") # Error Event pub
        
        self.app_state.recent_messages = list(reversed(cli_messages))
        self.event_manager.publish("MESSAGES_UPDATED") # Messages updated Event pub
        return True

    async def send_message(self, content: str) -> bool:
        """현재 채널에 메시지를 전송하고 성공 여부를 반환합니다."""
        if not self.app_state.current_channel:
            self.event_manager.publish("ERROR", "[오류] 메시지를 보낼 채널이 선택되지 않았습니다. 채널을 설정해 주세요.") # Error Event pub
            return False
        
        try:
            message = await self.app_state.current_channel.send(content)
            self.event_manager.publish("MESSAGE_SENT_SUCCESS", message) # Message sent success Event pub
            return True
        except discord.errors.Forbidden:
            self.event_manager.publish("ERROR", "[오류] 채널에 메시지를 보낼 권한이 없습니다. 봇 역할 권한을 확인해 주세요.") # Error Event pub
        except Exception as e:
            self.event_manager.publish("ERROR", f"[오류] 메시지 전송 실패: {e}") # Error Event pub
        return False

    async def send_file(self, file_path: str, content: str | None = None) -> bool:
        """지정된 파일을 현재 채널에 전송하고 성공 여부를 반환합니다."""
        if not self.app_state.current_channel:
            self.event_manager.publish("ERROR", "[오류] 파일을 보낼 채널이 선택되지 않았습니다. 채널을 설정해 주세요.") # Error Event pub
            return False
        
        if not os.path.exists(file_path):
            self.event_manager.publish("ERROR", f"[오류] 파일을 찾을 수 없습니다: '{file_path}'") # Error Event pub
            return False
            
        try:
            discord_file = discord.File(file_path)
            message = await self.app_state.current_channel.send(content=content, file=discord_file)
            self.event_manager.publish("FILE_SENT_SUCCESS", message) # File sent success Event pub
        except discord.errors.Forbidden:
            self.event_manager.publish("ERROR", "[오류] 채널에 파일을 첨부할 권한이 없습니다. 봇 역할 권한을 확인해 주세요.") # Error Event pub
        except Exception as e:
            self.event_manager.publish("ERROR", f"[오류] 파일 전송 실패: {e}") # Error Event pub
        return False
